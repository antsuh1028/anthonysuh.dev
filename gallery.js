...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.plane.scale.x = (this.viewport.width * (290 * this.scale)) / this.screen.width;
...
class App {
  constructor(container, { items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Inter', scrollSpeed = 2, scrollEase = 0.05, autoDrift = 0.02 } = {}) {
  ...
  update() {
    // gentle idle drift so the background feels alive
    if (!this.isDown) this.scroll.target += this.autoDrift;
  ...
  addEventListeners() {
    ...
    // scoped to the container so it doesn't hijack the rest of the page (or the Mii)
...
if (container) {
  // Wait for Inter (already loaded by the page) so canvas titles use it
  const start = () => new App(container, {
    items: [
      { image: './images/grad.jpg', text: 'graduation' },
      { image: './images/JP2.jpg', text: 'tokyo' },
      { image: './images/profile2.jpg', text: 'seattle' },
      { image: './images/image0.jpg', text: 'skytree' },
      { image: './images/profile3.jpg', text: 'downtown' }
    ],
    bend: 0.8, // lower bend = wider, flatter arc
    textColor: '#ccd6f6',
    borderRadius: 0.05,
